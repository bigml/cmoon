...
#define _COL_EMAP_WHO "sender, count(sender), "             \
...
NEOERR* system_who_data_get(CGI *cgi, HASH *dbh, HASH *evth, session_t *ses)
{
    ...
    MDB_QUERY_RAW(db, "emap", _COL_EMAP_WHO,
                  " %s GROUP BY sender ORDER BY count(sender) DESC LIMIT 100",
                  NULL, str.buf);
...
#define _COL_EMAP_VIEW_DETAIL "sender, es_one, es_two, es_three, "  \
